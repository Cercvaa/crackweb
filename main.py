# ---- Windows-pickle → Linux fix ----
import pathlib, pickle, types
...
_pathfix_pickle = types.SimpleNamespace(load=_pathfix_load, Unpickler=_PathFixUnpickler)
# ------------------------------------
...
MODELS_DIR = APP_DIR / "models"; MODELS_DIR.mkdir(exist_ok=True)
MODEL_LOCAL = MODELS_DIR / "crack_classifier.pkl"
MODEL_URL = os.getenv("MODEL_URL")

def ensure_model():
    if MODEL_LOCAL.exists(): return
    if not MODEL_URL: raise RuntimeError("MODEL_URL env var not set")
    print(f"Downloading model from {MODEL_URL} ...", flush=True)
    r = requests.get(MODEL_URL, timeout=300); r.raise_for_status()
    MODEL_LOCAL.write_bytes(r.content)

ensure_model()
learn = load_learner(MODEL_LOCAL, pickle_module=_pathfix_pickle)
